...
class DataTrainingArguments:
    ...
    max_samples: Optional[int] = field(
        default=0,
        metadata={"help": "Max samples to consider."},
    )
...
def main():
    ...
    def remove_special_characters(batch, train=True):
        batch["text"] = (
            re.sub(chars_to_ignore_regex, "", batch["sentence"])
            .lower()
            .strip()
        )
        if train:
    ...
    if data_args.max_samples: train_dataset = train_dataset.select(range(data_args.max_samples))
    ...
    if training_args.do_eval:
        ...
        if data_args.max_samples: eval_dataset = eval_dataset.select(range(data_args.max_samples))
        # TODO: set up ValidationDataLogger
    ...
    if data_args.max_samples: test_dataset = test_dataset.select(range(data_args.max_samples))
    # TODO: set up another ValidationDataLogger for test data
    ...
    def compute_metrics(pred):
        ...
        # TODO: log predictions with `ValidationDataLogger`
    ...
    if loss_nan_stopping_callback.stopped:
        ...
        # TODO: log predictions with `ValidationDataLogger` (this time for the test dataset)
